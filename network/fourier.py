"""Fast Fourier Convolution blocks used by the LSFSeg image encoder.
...
sys.path.append(os.path.dirname(__file__))  # Keep local fallback imports available.
...
try:
    from network.spatial_transform import LearnableSpatialTransformWrapper
except ModuleNotFoundError:

    class LearnableSpatialTransformWrapper(nn.Module):
        """Fallback used when optional kornia spatial transforms are unavailable."""

        def __init__(self, impl, **_kwargs):
            super().__init__()
            self.impl = impl
...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        hidden_channels = max(1, int(channel) // int(reduction))
...
class FFCSE_block(nn.Module):
    def __init__(self, channels, ratio_g):
        ...
        r = 16
        hidden_channels = max(1, int(channels) // r)
        ...
        self.conv_a2l = (
            None
            if in_cl == 0
            else nn.Conv2d(hidden_channels, in_cl, kernel_size=1, bias=True)
        )
        self.conv_a2g = (
            None
            if in_cg == 0
            else nn.Conv2d(hidden_channels, in_cg, kernel_size=1, bias=True)
        )
...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        # Step 3: split into real/imag channels
        ffted = torch.stack((ffted.real, ffted.imag), dim=-1)

        # Step 4: permute to (B, C, 2, H, W/2+1), then flatten the complex axis
        ...
        # Step 5: optional spectral position encoding
        ...
        # Step 7: 1x1 convolution
        ffted = self.conv_layer(ffted)  # (batch, c*2, h, w/2+1)
        ffted = self.relu(self.bn(ffted))

        # Step 8: reshape back to complex
        ffted = (
            ffted.view(
                (
                    batch,
        ...
        # Step 10: rescale back if needed
        if self.spatial_scale_factor is not None:
...
class SpectralTransform(nn.Module):
    def __init__(
        ...
        **fu_kwargs,
        ...
        if self.enable_lfu:
            self.lfu = FourierUnit(out_channels, out_channels // 2, groups)
